...
/**
 * Utility class, which will have independent utility methods, that can be used
 * by other classes.
 * 
...
public class FileUtil {
	 ...
	 * shortname. It supports User Principle Name (UPN) and Full Distinguished
	 * Name (DN) format.
	 * 
	 * @param longName Username in the form of UPN or Full DN format.
	 * @return ShortName of the Username (Which may be in one of the form i.e.
	 *         UPN or Full DN format.)
	 */
	 ...
	 * @return Return the Host Name or Host IP. Returns null, if URL is in
	 *         malformed state.
	 */
	public static String getHost(String strURL) {
		try {
			...
			LOGGER.log(Level.WARNING, "Malformed URL has occurred. Either no legal protocol could be found in a URL string or the URL string could not be parsed. URL string is: ["
					+ strURL + "]", e);
			return null;
		}
	}
	...
	public static String getFQHN(String host) {
		try {
			...
			LOGGER.log(Level.WARNING, "Unable to reach the Host: [" + host
					+ "]", e);
			return null;
		}
	}
	...
	/**
	 * Takes the Calendar instance and fetches the current timezone offset in
	 * the format (+|-)hhmm. Inserts the ":" in the above format and returns the
	 * timezonezone offset in the format (+|-)hh:mm
	 * 
	...
	public static String getTimeZone(Calendar cal) {
		...
		StringBuffer strDate = new StringBuffer(
				dateStandard.format(cal.getTime()));
